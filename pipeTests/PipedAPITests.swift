...
struct PipedAPITests {
    ...
    @Test func streamsDecodesSubtitles() async throws {
        let json = """
        {"title":"T","description":null,"uploader":"U","uploaderUrl":null,"duration":10,"hls":null,"audioStreams":[],"videoStreams":[],"thumbnailUrl":"th","uploadDate":null,"subtitles":[{"url":"https://x/en.ttml","mimeType":"application/ttml+xml","name":"English","code":"en","autoGenerated":false}]}
        """
        try await withStub(json) {
            let s = try await PipedAPI.streams("v")
            #expect(s.subtitles?.count == 1)
            #expect(s.subtitles?.first?.code == "en")
            #expect(s.subtitles?.first?.autoGenerated == false)
        }
    }

    @Test func streamsSubtitlesNilWhenAbsent() async throws {
        try await withStub(Self.streamJSON) {
            let s = try await PipedAPI.streams("v")
            #expect(s.subtitles == nil)
        }
    }
